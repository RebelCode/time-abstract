...
trait DurationAwareTrait
{
     ...
     * @since [*next-version*]
     *
     * @param int|string|Stringable $duration The number of seconds since unix epoch. Negative numbers are allowed.
     */
    protected function _setDuration($duration)
    {
        try {
            $this->duration = $this->_sanitizeTimestamp($duration);
        } catch (InvalidArgumentException $exception) {
            throw $this->_createInvalidArgumentException(
                $this->__('Argument is not a valid duration'),
                null,
                $exception,
                $duration
            );
